...
def mkmov_simple(starti=0,endi=400,length=10):
    output_directory = "/scratch/qkt4685/harmpi3D_0.9spin/img"
    ...
    for i in np.arange(starti,endi+1):
        ...
        cs, cb = plco(np.log10(rho),levels=np.linspace(-8,0,100),isfilled=1,k=0,xy=1,xmax=50,ymax=20,dobh=1,cb=1,extend="both",pretty=1)
        ax = plt.gca()
        ...
        plt.xlim(0,70);plt.ylim(-40,40)
        plt.draw();
        plt.savefig(os.path.join(output_directory, "frame%03d.png" % i))
